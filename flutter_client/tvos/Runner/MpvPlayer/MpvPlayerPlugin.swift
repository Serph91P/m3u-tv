// Method/event channel plugin for `m3u_tv/apple_mpv` +
// `m3u_tv/apple_mpv/events` on tvOS.
//
// Owns a registry of MpvPlayerCore instances keyed by the Dart-generated
// `viewId` (see lib/playback/apple_mpv_native_backend.dart, shared with
// iOS). The core for a given viewId is created by
// MpvPlayerPlatformViewFactory when Flutter instantiates the UiKitView,
// which can race with the Dart adapter's `load()` method call arriving
// first -- `load` retries briefly if the core isn't registered yet rather
...
final class MpvPlayerPlugin: NSObject, FlutterStreamHandler, MpvPlayerCoreDelegate {
  ...
  static let eventChannelName = "m3u_tv/apple_mpv/events"
